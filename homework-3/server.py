...
# то это один пользователь)
    ...
    return f"<p><b>Дата:</b> {datetime.now().strftime('%d/%m/%Y')}</p> <p><b>Время:</b>" \
           ...
           f"<p><b>Количество пользователей:</b> {len(users)}</p> <p><b>Пользователи:</b> {', '.join(users_real)}</p>" # количество сообщений и количество пользователей
...
def send_message():
    ...
    if ('/anonim' in text) and (str(name) not in a):
        a.append(name)
        message = {
            'time': time.time(),
